...
public class InsertionSort {
    ...
    public static ArrayList<Personagem> Insertionsort(ArrayList<Personagem> DB, Log log) {
        int tamanho = DB.size();
        for (int i = 1; i < tamanho; i++) {
            int j = i;
            LocalDate currentDate = DB.get(j).getDateOfBirth();
            LocalDate previousDate = DB.get(j - 1).getDateOfBirth();
            while (j > 0 && (currentDate.isBefore(previousDate) || (currentDate.isEqual(previousDate) && DB.get(j).getName().compareTo(DB.get(j - 1).getName()) < 0))) {
                Collections.swap(DB, j, j - 1);
                j--;
                if (j > 0) {
                    currentDate = DB.get(j).getDateOfBirth();
                    previousDate = DB.get(j - 1).getDateOfBirth();
                }
            }
        }
        return DB;
    ...
    public static void main(String[] args)throws Exception {
        ...
        log.pesquisa("801778_insercao.txt", timer);
